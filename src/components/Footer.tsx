            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {['Accueil', 'À propos', 'Compétences', 'Projets', 'Contact'].map(l => (
                <a key={l} href={`#${l.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')}`}
                  style={{ fontFamily: 'var(--font-sub)', fontSize: 14, color: '#64748B', textDecoration: 'none', transition: 'color 0.15s' }}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[
                ...
                <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  {item.href
                    ? <a href={item.href} style={{ fontFamily: 'var(--font-sub)', fontSize: 13.5, color: '#64748B', textDecoration: 'none', transition: 'color 0.15s' }}
                        onMouseEnter={e => (e.currentTarget as HTMLElement).style.color = '#fff'}
        ...
        <div style={{ borderTop: '1px solid #1E293B', paddingTop: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '1.5rem' }}>
            <a href="https://wa.me/22961642237" target="_blank"
              style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontFamily: 'var(--font-sub)', fontSize: 13, color: '#64748B', textDecoration: 'none', transition: 'color 0.15s' }}
